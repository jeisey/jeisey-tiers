...
- **Both market identity "bridges" terminate at the same roster-scoped registry.** `build_registry` builds the canonical player set from the current season's roster, and both `registry.lookup` calls in `_resolve_one_quote` end there, so a player the market prices but who is on no NFL roster cannot resolve through *either* bridge and cannot be aliased either (`alias_target_unknown`). Two bridges guard against a wrong answer, not against an absent player. Before reading a market coverage number as an identity problem, check whether the player exists on a roster at all.
- **The board and the market do not share a universe.** The intrinsic board projects anyone with NFL history; the market layer only knows current-roster players. Every "unpriced top-150 player" is one of: unrostered (the board is wrong), priced only in a cohort the block may not use (a rule question), or not priced by the vendor (nothing to fix). ADR-054 has the worked example; classify before remedying.
...
**The daily refresh is red and stays red until an owner decision.** Since 2026-08-26 the scheduled refresh fails `arbitrage.top_board_priced` — `HALF/redraft-10` prices 141 of its top 150, and the bar is 95%. **ADR-054 accounts for all nine misses individually** and the short version is that none of them is reachable by identity work: three are priced by MFL but are on no NFL roster, so no canonical player exists to resolve them to; three are priced only in `ppr-no-keeper`, which a HALF board may not use; three are not priced by MFL at all. Last-known-good is holding and the site keeps serving its most recent good build, which is the designed behaviour, not a second fault.

The decision ADR-054 puts to the owner is **the board universe, not the gate**: four of the nine are free agents that the board ranks inside the top 126 of a ten-team league (Stefon Diggs at fair rank 77). The market is right not to price them. Restricting the published board to rostered players would fix most of the shortfall, but it renumbers every fair rank, so it is a contract change and is not made unilaterally.

Two things that are *not* the cause, both checked and both worth not re-checking: nothing in the pipeline changed between the 2026-08-25 pass and the 2026-08-26 failure — the board reprojects daily and one more unpriceable player crossed into a top 150 — and the falling market identity-resolution rate (86% → 77%) is IDP and team defenses entering the aggregate, which this project does not model. ADR-052 flagged that drift as a thing to watch; it is benign.
...
**Decide the ADR-054 board-universe question.** It is the only thing standing between the pipeline and a green daily refresh, and every option is written up with its cost.
...
**Three ADRs are waiting for an owner decision** and should be read before any market work: **ADR-052** (market-data confidence is self-resolving; do not re-specify `min_total_drafts` to make it pass) **ADR-053** (the free-source sweep, and what each candidate would and would not fix), and **ADR-054** (the board ranks unrostered free agents; that, not the market, is what fails the coverage gate).
