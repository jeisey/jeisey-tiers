...
| Draft rail (mobile) | At 390px the rail overran its minimum width into the gap column and drew a line through the sentence explaining it; player names ran under the first anchor. Both because the columns were derived from the viewport while the rail floor and the name truncation were fixed constants. | The gap column is reserved and never encroached on; when space is tight the *name* column gives way instead. Name truncation is derived from the column width using the measured advance of the face, and at compact sizes the rail shows the surname — which is what fits and what a drafter reads. A geometric end-to-end assertion now fails if a name overlaps an anchor or a mark overlaps the gap text. |
