...
> - `e2e` — Playwright over the built sites (root, `/jeisey-tiers/`, and the degraded-artifact and lifecycle scenarios), then `verify:board` against five of them: the root build, the matured-market build, the in-season build and both ADR-079 windows. The matured build is the only fixture with a non-null `market_trend`, so it is the only one that exercises the Trend column and the chart's slope readout; the in-season build is the only one where `view=auto` resolves to the ROS board rather than the Tier Board, which is the transition the 2026-09-15 refresh met in production because no gate had been aimed here (ADR-084).
>
...
| `verify:board` | every rendered tier row, chart mark, arbitrage row, status badge, **the rest-of-season row where a build published one**, and the player card's retained market history — per published market, plus the cross-market overlay — against the bytes the site served |
