...
Snapshot `2026-08-20T14-11-48Z` retrieved 2026-08-20T14:11:48Z, source `myfantasyleague_adp`. Rule `phase5_cohort_v1` (ADR-039), frozen before this measurement existed.
...
|---|---:|---|---|---|---|---|
| HALF | 10 | `unfiltered` | `—` | no | yes | widest sufficient candidate |
| HALF | 12 | `unfiltered` | `—` | no | yes | widest sufficient candidate |
| HALF | 14 | `unfiltered` | `—` | no | yes | widest sufficient candidate |
| PPR | 10 | `ppr` | `IS_PPR=1` | no | yes | most specific sufficient candidate |
| PPR | 12 | `ppr` | `IS_PPR=1` | no | yes | most specific sufficient candidate |
| PPR | 14 | `ppr` | `IS_PPR=1` | no | yes | most specific sufficient candidate |
| STD | 10 | `unfiltered` | `—` | no | yes | widest sufficient candidate |
| STD | 12 | `unfiltered` | `—` | no | yes | widest sufficient candidate |
| STD | 14 | `unfiltered` | `—` | no | yes | widest sufficient candidate |

HALF-PPR can never be exact on this source: MFL exposes `IS_PPR` as a boolean and publishes no half-PPR filter (ADR-039).

## Composition caveat

The unfiltered aggregate carries 426 drafts, of which 401 are PPR (94%) and 25 are non-PPR. It therefore serves HALF, STD with a price set overwhelmingly by PPR drafters.

That is a *directional* approximation, not merely a wide one: PPR drafting lifts pass-catching backs and high-target receivers relative to standard scoring, so a standard-scoring reader should expect this board to read those players as slightly more expensive than their own league would. The rule still prefers it to the `std` cohort's 25 drafts, and every row says `cohort_approximate`, but the direction is worth stating rather than leaving to be inferred.
